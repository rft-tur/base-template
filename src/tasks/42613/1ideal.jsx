...
const Step1 = ({ onNext }) => {
  ...
  const validateDomain = () => {
    ...
    if (parts.length !== 2 || (parts.length==2 && !allowedTLDs.includes(parts[1])) ) {
  ...
  const checkDomain = () => {
    ...
    if (Math.random() > 0.5) {
      ...
      toast({ title: "Success", description: "Domain lookup successful" });
...
const Step2 = ({ domain, dnsProvider, onNext }) => {
  ...
  const checkMXRecords = () => {
    ...
    if (Math.random() > 0.5) {
      toast({ title: "Success", description: "MX records set" });
      onNext();
    } else {
...
const Step3 = ({ onNext }) => {
  ...
  const checkSPFRecord = () => {
    ...
    if (Math.random() > 0.5) {
      toast({ title: "Success", description: "SPF records set" });
      onNext();
    } else {
...
const Step4 = ({ onNext }) => {
  ...
  const checkDKIMRecord = () => {
    ...
    if (Math.random() > 0.5) {
      toast({ title: "Success", description: "DKIM records set" });
      onNext();
    } else {
